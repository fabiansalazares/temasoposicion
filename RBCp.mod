...
parameters 
    ...
    rhoA    ${\rho_A}$  (long_name='persistence TFP shock')
...
rhoA=0.97;              //technology autocorrelation base on linearly detrended Solow residual
rhog=0.97;
...
c^(-sigma)=beta/gammax*c(+1)^(-sigma)*
    (alpha*exp(A(+1))*(k/l(+1))^(alpha-1)+(1-delta));
[name='Labor FOC']
psi*c^sigma*1/(1-l)=w;
...
w=(1-alpha)*y/l;
[name='annualiAed real interest rate/firm FOC capital']
r=4*alpha*y/k(-1);
...
stoch_simul(order=1,irf=40,hp_filter=1600, tex) log_y log_k log_c log_l log_w r A ghat;
